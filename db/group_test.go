...
func testGroupToManyUsers(t *testing.T) {
	...
	_, err = tx.Exec("insert into \"group_user_assignment\" (\"group_id\", \"user_id\") values ($1, $2)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"group_user_assignment\" (\"group_id\", \"user_id\") values ($1, $2)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}
...
func testGroupToManyLocations(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	_, err = tx.Exec("insert into \"location_group_assignment\" (\"group_id\", \"location_id\") values ($1, $2)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"location_group_assignment\" (\"group_id\", \"location_id\") values ($1, $2)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := GroupSlice{&a}
	if err = a.L.LoadLocations(ctx, tx, false, (*[]*Group)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testGroupToManyAddOpLocations(t *testing.T) {
	var err error
...
func testGroupToManySetOpLocations(t *testing.T) {
	var err error
...
func testGroupToManyRemoveOpLocations(t *testing.T) {
	var err error
...
var (
	groupDBTypes = map[string]string{`ID`: `uuid`, `LocalID`: `character varying`, `DisplayName`: `character varying`}
	_            = bytes.MinRead
)
