...
func testUserToManyGroups(t *testing.T) {
	...
	_, err = tx.Exec("insert into \"group_user_assignment\" (\"user_id\", \"group_id\") values ($1, $2)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"group_user_assignment\" (\"user_id\", \"group_id\") values ($1, $2)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}
...
func testUserToManyLocations(t *testing.T) {
	...
	_, err = tx.Exec("insert into \"location_user_assignment\" (\"user_id\", \"location_id\") values ($1, $2)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"location_user_assignment\" (\"user_id\", \"location_id\") values ($1, $2)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}
...
var (
	userDBTypes = map[string]string{`ID`: `uuid`, `LocalID`: `character varying`, `DisplayName`: `character varying`, `Username`: `character varying`}
	_           = bytes.MinRead
)
