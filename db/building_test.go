...
func testBuildingToManyLocations(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = a.L.LoadLocations(ctx, tx, false, (*[]*Building)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testBuildingToManyAddOpLocations(t *testing.T) {
	var err error
...
var (
	buildingDBTypes = map[string]string{`ID`: `uuid`, `Name`: `character varying`}
	_               = bytes.MinRead
)
